...
Edit the config.json file in the root directory and add your API keys. If an API key is missing, the tool will use a free alternative (if available) or skip the module.
...
The tool uses command-line flags for individual modules. Use the --all flag to run a full scan.
...
python cssltd_webosint.py -d example.com --all -o output.json
